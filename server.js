...
if (!AIRTABLE_API_KEY || !AIRTABLE_BASE_ID) {
  console.error('Missing required env vars: AIRTABLE_API_KEY and AIRTABLE_BASE_ID');
  console.error('Copy .env.example to .env and fill in your Airtable credentials.');
  process.exit(1);
}
...
// STF uses "Completed" (with 'd'), Trailblazers uses "Complete" — normalise to "Complete"
...
async function fetchTrailblazers(baseId, headers) {
  const filter = 'OR({Y1 Completion Status} = "Complete", {Y1 Completion Status} = "In Progress")';
  const records = await fetchAirtablePages(baseId, AIRTABLE_TABLE_NAME, filter, headers);
...
app.listen(Number(API_PORT), () => {
  console.log(`API server running → http://localhost:${API_PORT}`);
